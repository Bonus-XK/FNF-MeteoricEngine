         ...
         if (FileSystem.exists(dev_path))
         {
            ...
            for(file in files)
            {
               if (extract_version.match(file))
               {
                  ...
                  var v0 = Std.parseFloat(split_ver[0]);
                  var v1 = split_ver.length > 1 ? Std.parseFloat(split_ver[1]) : 0;
                  var b0 = Std.parseFloat(split_best[0]);
                  var b1 = split_best.length > 1 ? Std.parseFloat(split_best[1]) : 0;
                  if (v0 > b0 || (v0 == b0 && v1 > b1))
                     best = ver;
               }
            }
            if (best!="0.0")
