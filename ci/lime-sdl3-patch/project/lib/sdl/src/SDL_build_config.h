...
/* st_mtim 是 Linux 命名，mac 用 st_mtimespec（SDL_sysfsops.c 有 APPLE 分支） */
...
/* getauxval 是 Linux/Android API，mac 无 sys/auxv.h */
#if !defined(SDL_PLATFORM_MACOS) && !defined(SDL_PLATFORM_IOS)
...
/* ============ mac 平台驱动配置覆盖（SDL3 移植） ============ */
#if defined(SDL_PLATFORM_MACOS)
/* 音频：CoreAudio；关闭 android 驱动 */
...
/* 摄像头：AVFoundation(CoreMedia) */
#undef SDL_CAMERA_DRIVER_ANDROID
#define SDL_CAMERA_DRIVER_COREMEDIA 1
/* 对话框：Cocoa */
#define SDL_DIALOG_COCOA 1
/* 系统托盘：Cocoa */
#define SDL_TRAY_COCOA 1
/* 平台杂项（url 打开等）由 SDL_PLATFORM_MACOS 自动启用 */
#endif /* SDL_PLATFORM_MACOS */
/* ============ mac 平台驱动配置结束 ============ */

/* ============ windows 平台驱动配置覆盖（SDL3 移植） ============ */
...
/* 视频：Windows + OpenGL(WGL) */
#undef SDL_VIDEO_DRIVER_ANDROID
#define SDL_VIDEO_DRIVER_WINDOWS 1
/* 摇杆：DINPUT + XINPUT + RAWINPUT（保留 HIDAPI/VIRTUAL） */
#undef SDL_JOYSTICK_ANDROID
...
/* 对话框/托盘：Windows */
#define SDL_DIALOG_WINDOWS 1
#define SDL_TRAY_WINDOWS 1
/* 加载/线程/时间/时钟：Windows */
#undef SDL_LOADSO_DLOPEN
#define SDL_LOADSO_WINDOWS 1
/* POSIX/BSD 专用宏（mingw 无这些 API） */
