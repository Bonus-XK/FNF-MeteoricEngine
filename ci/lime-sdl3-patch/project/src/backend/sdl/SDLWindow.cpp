...
namespace lime {
	...
	// 【Meteoric 高清渲染】进程级状态：可执行文件旁 meteoric_dpi_mode.txt 首字符 '0' → 1x 高性能；
	// 默认/缺失/其他 → 高清（Windows 按显示器缩放渲染、macOS Retina 2x）。
	// 启动/构造时各读一次并缓存，避免每帧 I/O。
	static bool meteoricHighDpiEnabled = true;


	bool SDLWindow::MeteoricHighDpiEnabled () {
		...
		if (basePath != NULL) {
			...
			if (modeFile != NULL) {
				...
				fclose (modeFile);
				meteoricHighDpiEnabled = (modeChar != '0');

			}

		}

		return meteoricHighDpiEnabled;

	}


	double SDLWindow::MeteoricScale (SDL_Window* window) {

		if (!meteoricHighDpiEnabled || window == NULL) return 1.0;
	...
	bool SDLWindow::MeteoricScaledWindowFits (int logicalWidth, int logicalHeight) {

		// 在 SDL_Init 之前调用（进程级 DPI 感知必须在初始化前定），此时还没有窗口，
		// 只能按主显示器判断；SDL3 允许 SDL_GetPrimaryDisplay/UsableBounds 在 Init 前给出
		// 尽力而为的答案，拿不到就保守当作"放不下"（退回逻辑尺寸窗口 = 旧版行为，画面必定完整）。
		SDL_DisplayID display = SDL_GetPrimaryDisplay ();
		if (display == 0) return false;

		double scale = SDL_GetDisplayContentScale (display);
		if (!(scale > 0.0)) return false;
		if (scale <= 1.0) return true; // 无需放大，不存在溢出问题

		SDL_Rect usable;
		if (!SDL_GetDisplayUsableBounds (display, &usable)) return false;

		// 留 8% 余量：任务栏/标题栏/边框会吃掉实际可视区域，
		// 贴边刚好塞下的窗口在真机上一样会被裁。
		int maxW = (int) (usable.w * 0.92);
		int maxH = (int) (usable.h * 0.92);

		int physicalW = (int) (logicalWidth * scale + 0.5);
		int physicalH = (int) (logicalHeight * scale + 0.5);

		return physicalW <= maxW && physicalH <= maxH;
	...
	SDLWindow::SDLWindow (Application* application, int width, int height, int flags, const char* title) {
		...
		// 【Meteoric 高清渲染】一次性读取模式文件（'0'=1x 高性能；默认=高清），
		// macOS 控制 HIGH_PIXEL_DENSITY 标志，Windows 控制窗口放大/DPI 感知/坐标折算。
		bool highDpiAllowed = SDLWindow::MeteoricHighDpiEnabled ();
		...
		if (flags & WINDOW_FLAG_HARDWARE) {
			...
			if ((flags & WINDOW_FLAG_ALLOW_HIGHDPI) && highDpiAllowed)
		...
		#ifdef HX_WINDOWS
		// 【Meteoric 修复】Windows 高清渲染不生效：
		// SDL3 在 Windows 上不做「逻辑尺寸/物理尺寸」归一化（SDL_GetWindowSize 与
		// SDL_GetWindowSizeInPixels 都返回物理像素），窗口默认按 1280×720 物理像素创建，
		// 高分屏上窗口偏小、放大/全屏时 OpenFL 拿不到 DPI 缩放而拉伸模糊（与 macOS 关高清同观感）。
		// 高清开启时把窗口放大到 逻辑尺寸 × 显示器缩放，并配合 GetWidth/GetHeight/GetScale
		// 向 Haxe 报告逻辑尺寸：OpenFL 即按物理分辨率渲染（与 macOS Retina 行为对齐）。
		if (highDpiAllowed) {

			double scale = SDLWindow::MeteoricScale (sdlWindow);
			if (scale > 1.0) {

				int physicalW = (int)(width * scale + 0.5);
				int physicalH = (int)(height * scale + 0.5);
				if (physicalW != width || physicalH != height)
					SDL_SetWindowSize (sdlWindow, physicalW, physicalH);
		...
		// SDL3 移除了 SDL_GetWindowWMInfo / SDL_SysWMinfo；窗口图标改用 SDL_SetWindowIcon（见 SetIcon）
		#endif
		...
		if (flags & WINDOW_FLAG_HARDWARE) {
			...
			// SDL3: SDL_CreateRenderer 自动选择加速/软件渲染，不再使用 SDL_RENDERER_ACCELERATED 等 flags

			#ifdef EMSCRIPTEN
			// SDL_RENDERER_PRESENTVSYNC 已移除（SDL3 用 SDL_SetRenderVSync）
			#endif
			...
			// SDL3: SDL_GL_MakeCurrent 返回 bool（true=成功）；SDL2 返回 int（0=成功）
			bool mc = context ? SDL_GL_MakeCurrent (sdlWindow, context) : false;
	...
	void SDLWindow::Alert (const char* message, const char* title) {
		...
		#if defined (HX_WINDOWS) && !defined (HX_WINRT)
		// SDL3 移除了 SDL_GetWindowWMInfo（任务栏闪烁暂不支持）
		#endif
	...
	void* SDLWindow::ContextLock (bool useCFFIValue) {
		...
		if (sdlRenderer) {
			...
			if (useCFFIValue) {
				...
				// SDL3: SDL_LockTexture 返回 bool（true=成功）
	...
	const char* SDLWindow::GetContextType () {
		...
		if (context) {
			...
			if (name && strcmp (name, "software") == 0) {
	...
	void SDLWindow::GetDisplayMode (DisplayMode* displayMode) {
		...
		const SDL_DisplayMode* modePtr = SDL_GetCurrentDisplayMode (SDL_GetDisplayForWindow (sdlWindow));
		if (modePtr) mode = *modePtr;
	...
	int SDLWindow::GetHeight () {
		...
		#ifdef HX_WINDOWS
		// 【Meteoric 修复】Windows 高清：物理像素 → 逻辑尺寸（与 GetWidth 对称）。
		double scale = SDLWindow::MeteoricScale (sdlWindow);
		if (scale > 1.0) height = (int)(height / scale + 0.5);
		#endif
	...
	float SDLWindow::GetOpacity () {
		...
		opacity = SDL_GetWindowOpacity (sdlWindow);
	...
	double SDLWindow::GetScale () {
		...
		// 【Meteoric 修复】Windows 高清：SDL3 在 Windows 上 window 尺寸即物理像素，
		// 旧实现 SDL_GetWindowSizeInPixels/SDL_GetWindowSize 恒为 1.0 → OpenFL 永远按
		// 逻辑尺寸渲染（高分屏模糊/偏小）。SDL_GetWindowDisplayScale 是 SDL3 官方「内容缩放」：
		//   Windows = 显示器 DPI/96（100%→1.0，125%→1.25，150%→1.5，200%→2.0）；
		//   macOS Retina = 2.0（开 HIGH_PIXEL_DENSITY）/ 1.0（关）—— 与旧行为完全一致。
		return SDLWindow::MeteoricScale (sdlWindow);
	...
	int SDLWindow::GetWidth () {
		...
		#ifdef HX_WINDOWS
		// 【Meteoric 修复】Windows 高清：先把物理像素折算回逻辑尺寸再报告给 Haxe，
		// 使 OpenFL 的 stage 保持 1280×720 逻辑坐标（与 macOS Retina 一致）。
		double scale = SDLWindow::MeteoricScale (sdlWindow);
		if (scale > 1.0) width = (int)(width / scale + 0.5);
		#endif
	...
	void SDLWindow::ReadPixels (ImageBuffer *buffer, Rectangle *rect) {
		...
		if (sdlRenderer) {
			...
			// SDL3: SDL_RenderReadPixels 返回 SDL_Surface*，需自行复制像素
			SDL_Surface* readSurface = SDL_RenderReadPixels (sdlRenderer, &bounds);
			if (readSurface) {

				buffer->Resize (readSurface->w, readSurface->h, 32);
				int copyPitch = readSurface->w * 4;
				int stride = buffer->Stride ();
				Uint8* dst = (Uint8*)buffer->data->buffer->b;
				const Uint8* srcPixels = (const Uint8*)readSurface->pixels;

				if (copyPitch <= stride) {

					for (int y = 0; y < readSurface->h; y++) {

						memcpy (dst + y * stride, srcPixels + y * readSurface->pitch, copyPitch);

					}

				}

				SDL_DestroySurface (readSurface);
	...
	void SDLWindow::Resize (int width, int height) {
		...
		#ifdef HX_WINDOWS
		// 【Meteoric 修复】Windows 高清：Haxe 侧 resizeWindow 传的是逻辑尺寸，
		// 需先乘回物理像素（与构造时的窗口放大逻辑对称），否则窗口会被缩成 1/scale。
		double scale = SDLWindow::MeteoricScale (sdlWindow);
		if (scale > 1.0) {
	...
	void SDLWindow::SetDisplayMode (DisplayMode* displayMode) {
		...
		SDL_DisplayMode mode = {};
		mode.format = (SDL_PixelFormat)pixelFormat;
		mode.w = displayMode->width;
		mode.h = displayMode->height;
		mode.refresh_rate = displayMode->refreshRate;
	...
	void SDLWindow::SetIcon (ImageBuffer *imageBuffer) {
		...
		SDL_Surface *surface = SDL_CreateSurfaceFrom (imageBuffer->width, imageBuffer->height, SDL_PIXELFORMAT_ABGR8888, imageBuffer->data->buffer->b, imageBuffer->Stride ());
