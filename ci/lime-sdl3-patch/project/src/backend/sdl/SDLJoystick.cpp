...
namespace lime {
	...
	void SDLJoystick::Init () {
		...
		// SDL3: 用 SDL_GetJoysticks() 数组枚举（SDL2 的 SDL_NumJoysticks/SDL_JoystickNameForIndex 已移除）
		int joyCount = 0;
		SDL_JoystickID* ids = SDL_GetJoysticks (&joyCount);
		for (int i = 0; i < joyCount; i++) {

			SDL_Joystick* joy = SDL_GetJoystickFromID (ids[i]);
			if (joy && strstr (SDL_GetJoystickName (joy), "Accelerometer")) {

				accelerometer = SDL_OpenJoystick (ids[i]);
