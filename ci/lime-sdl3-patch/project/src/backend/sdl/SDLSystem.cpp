...
namespace lime {
	...
	bool Clipboard::SetText (const char* text) {
		...
		// SDL3: SDL_SetClipboardText 返回 bool（true=成功）
	...
	void* System::GetDisplay (bool useCFFIValue, int id) {
		...
		if (useCFFIValue) {
			...
			// SDL3：DisplayID 是不透明 ID（非 0..n-1 索引），用 SDL_GetDisplayName 判断有效性
			...
			float dpi = 72.0;
			// SDL3 移除了 SDL_GetDisplayDPI；暂用默认 72.0
			alloc_field (display, id_dpi, alloc_float (dpi));

			SDL_DisplayMode displayMode = {};
			DisplayMode mode;

			const SDL_DisplayMode* displayModePtr = SDL_GetDesktopDisplayMode (id);
			if (displayModePtr) displayMode = *displayModePtr;
			...
			// SDL3：DisplayID 是不透明 ID，用 SDL_GetDisplayName 判断有效性
			if (!SDL_GetDisplayName (id)) {
	...
	int System::GetNumDisplays () {
		...
		// SDL3: SDL_GetNumVideoDisplays 已移除，改用 SDL_GetDisplays() 数组
		int count = 0;
	...
	FILE* FILE_HANDLE::getFile () {
		...
		// SDL3: SDL_IOStream 是不透明结构，无法取出底层 FILE*（SDL2 的 hidden.stdio/androidio 已移除）。
		// 返回 NULL 后，上层（如 PNG 解码）会自动走"读入内存"路径，功能等价。
		return NULL;
	...
	bool FILE_HANDLE::isFile () {
		...
		// SDL3: IOStream 不透明，无类型判断（SDL2 的 type 字段已移除）；返回 false 走内存路径
		return false;
	...
	int fclose (FILE_HANDLE *stream) {
		...
		if (stream) {
			...
			System::GCEnterBlocking ();
			bool code = SDL_CloseIO ((SDL_IOStream*)stream->handle);
			delete stream;
			System::GCExitBlocking ();
			return code;
	...
	FILE_HANDLE *fdopen (int fd, const char *mode) {
		...
		// SDL3 移除了 SDL_IOFromFP；该函数当前无上层调用方，直接返回 NULL
		return NULL;
	...
	size_t fread (void *ptr, size_t size, size_t count, FILE_HANDLE *stream) {
		...
		// SDL3: SDL_ReadIO(ctx, ptr, size) 返回实际读取字节数（可能少于请求）
		Uint64 bytesRead = SDL_ReadIO (stream ? (SDL_IOStream*)stream->handle : NULL, ptr, size * count);
		nmem = (size > 0) ? (size_t)(bytesRead / size) : 0;
	...
	size_t fwrite (const void *ptr, size_t size, size_t count, FILE_HANDLE *stream) {
		...
		// SDL3: SDL_WriteIO(ctx, ptr, size) 返回实际写入字节数
		Uint64 bytesWritten = SDL_WriteIO (stream ? (SDL_IOStream*)stream->handle : NULL, ptr, size * count);
		nmem = (size > 0) ? (size_t)(bytesWritten / size) : 0;
