...
class ModInstaller
{
	...
	function pumpFolderCopy():Void
	{
		if (busy && copier != null)
		{
			...
			// 看门狗：拷贝线程卡死兜底（系统复制命令无逐文件进度，按调用方预估的
			// timeoutMs 判超时——20GB 级 mod 拷贝可达数分钟，固定 30 秒会误杀）
			if (copier != null && !copier.finished)
			{
				var now:Float = haxe.Timer.stamp();
				if (copier.total > 0 && now - lastCopierProgressAt > 90)
					abortTask('复制超时（90 秒无进展），已取消。');
				else if (copier.total <= 0 && now - copierStartedAt > copier.timeoutMs)
					abortTask('复制超时（' + Std.int(copier.timeoutMs / 1000) + ' 秒未完成），已取消。');
			}
		}
	}
	...
	{
		// 阶段标记：此后任何拖入路径上的崩溃/异常都能在 crash/last_stage.txt 定位到本入口
		try { CrashHandler.mark('ModInstaller:drop'); } catch (e:Dynamic) {}
		try
		{
		...
		{
			// 兜底：拖入路径上的任何异常都不允许逃进 C++ 事件分发（避免无弹窗闪退）
			trace('[ModInstaller] onDropFile error: ' + Std.string(e));
			try { showMessage('安装失败', '处理拖入文件时出错：\n' + Std.string(e)); } catch (e2:Dynamic) {}
		}
	}
	...
	function startZip(zipPath:String):Void
	{
		...
		var plan:ZipPlan = null;
		try
		{
			plan = ModZipPlanner.analyze(entries);
		}
		catch (e:Dynamic)
		{
			busy = false;
			showMessage('无法安装', '分析压缩包时出错：\n' + Std.string(e));
			return;
		}
	...
	function cleanupTemp():Void
	{
		if (tempRoot != null && tempRoot.length > 0)
		{
			...
			tempRoot = null;
			// 主线程同步删除临时目录：大 mod 解压出的几百个文件删除约一两秒，
			// 换取安装路径上不再有任何工作线程 Haxe 对象分配（hxcpp 线程 + GC 在
			// Release 下可致堆损坏 abort，见 FolderCopier.hx v2 注释）。
			try { deleteDirRecursive(p); } catch (e:Dynamic) {}
		}
	}
	...
	function installFolder(folderPath:String):Void
	{
		...
		try
		{
			// 后台线程执行系统复制命令（ditto / xcopy / cp -R），工作线程零 Haxe 对象分配
			// （修复：旧版工作线程逐文件 File.copy + 跨线程字符串消息，2.1GB 级谱面 JSON
			//  在 Release 下触发 malloc 堆损坏 abort，见 crash/native_crash_20260905_220547.txt）
			folderInstallName = targetName;
			copier = new FolderCopier();
			// 按目录字节数预估超时：30MB/s 下限 + 120 秒保底，上限 30 分钟
			var est:Null<Float> = FolderCopier.estimateTotalBytes(folderPath);
			if (est != null && est > 0)
				copier.timeoutMs = Std.int(Math.min(1800 * 1000, Math.max(120 * 1000, est / (30 * 1024 * 1024) * 1000)));
