...
static final VERT_HEADER:String = "		#pragma version
		#pragma precision
	...
	";
static final VERT_BODY:String = "		openfl_Alphav = openfl_Alpha;
	...
	";
static final FRAG_HEADER:String = "		#pragma version
		#pragma precision
		varying float openfl_Alphav;
		varying vec4 openfl_ColorMultiplierv;
		varying vec4 openfl_ColorOffsetv;
	...
	";
static final FRAG_BODY:String = "		vec4 color = texture2D(bitmap, openfl_TextureCoordv);
		gl_FragColor = color * openfl_Alphav;
	";
static final DEFAULT_FRAGMENT_SOURCE:String = "		#pragma header
		void main(void) {
			#pragma body
		}
	";
static final DEFAULT_VERTEX_SOURCE:String = "		#pragma header
		void main(void) {
			#pragma body
		}
	";
	...
	public function new(?fragmentSource:String, ?vertexSource:String):Void
	{
		if (fragmentSource != null && fragmentSource.length > 0)
			...
			glFragmentSource = DEFAULT_FRAGMENT_SOURCE;

		if (vertexSource != null && vertexSource.length > 0)
			glVertexSource = vertexSource;
		else
			glVertexSource = DEFAULT_VERTEX_SOURCE;
	...
	{
		if (value != null)
			value = value.replace("#pragma header", FRAG_HEADER).replace("#pragma body", FRAG_BODY);
	...
	{
		if (value != null)
			value = value.replace("#pragma header", VERT_HEADER).replace("#pragma body", VERT_BODY);
