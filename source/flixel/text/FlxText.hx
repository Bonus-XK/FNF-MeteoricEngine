...
class FlxText extends FlxSprite
{
	...
	// Meteoric：文字位图超采样密度（Retina/系统缩放 = window.scale，桌面 2 常见；非桌面恒 1）
	var _textDpi:Int = 1;

	override function get_width():Float
	{
		regenGraphic();
		return super.get_width() / _textDpi;
	}

	override function get_height():Float
	{
		regenGraphic();
		return super.get_height() / _textDpi;
	}
	...
	function regenGraphic():Void
	{
		...
		// Meteoric：物理 DPI 超采样（Retina=2 时位图 2 倍、字形按 2x 光栅化；显示尺寸不变）
		_textDpi = 1;
		#if desktop
		// 【Meteoric 修复】Windows 系统缩放常为 1.25/1.5（非整数）：Std.int 会截成 1，
		// 导致文字超采样失效（与「高清显示不生效」同源：window.scale 修复后为 1.25/1.5）。
		// 向上取整：1.25/1.5 → 2，2.0 → 2，与 macOS Retina 2x 同口径；上限 4 防极端位图爆炸。
		var _ws:Float = FlxG.stage.window != null ? FlxG.stage.window.scale : 1.0;
		_textDpi = (_ws > 1.0) ? Std.int(Math.ceil(_ws)) : 1;
		if (_textDpi < 1) _textDpi = 1;
		if (_textDpi > 4) _textDpi = 4;
		#end
		var dpi:Int = _textDpi;

		var newWidth:Int = Math.ceil(textField.width) * dpi;
		// Account for gutter
		var newHeight:Int = (Math.ceil(textField.textHeight) + VERTICAL_GUTTER) * dpi;
		...
		if (oldWidth != newWidth || oldHeight != newHeight)
		{
			...
			// Meteoric：位图 2 倍密度（显示尺寸由 draw() 绘制期按 1/dpi 补偿回逻辑值）；
			// 缩放绕中心会位移文本 → 文字绘制锚点固定为左上角（origin/offset 归零，
			// FlxText 本身不做旋转/居中轴缩放，锚点不影响布局语义）
			if (dpi > 1)
			{
				origin.set(0, 0);
				offset.set(0, 0);
			}
			textField.height = Std.int(newHeight / dpi) * 1.2;
			_flashRect.x = 0;
			_flashRect.y = 0;
	...
	function drawTextFieldTo(graphic:BitmapData):Void
	{
		...
		if (_textDpi > 1)
		{
			// Meteoric：把绘制矩阵整体放大 dpi 倍（cairo CTM 随之缩放 → 字形按 2x 光栅化，
			// 描边偏移仍按逻辑值由矩阵统一放大）；完整保存/恢复 6 个矩阵分量，
			// 避免缩放与位移组合不满足恒等导致描边/文字逐次偏移
			var ma:Float = _matrix.a, mb:Float = _matrix.b, mc:Float = _matrix.c,
				md:Float = _matrix.d, mtx:Float = _matrix.tx, mty:Float = _matrix.ty;
			_matrix.scale(_textDpi, _textDpi);
			_matrix.translate(-1, -1); // left and up（逻辑值；原版 -1,-1 语义）
			graphic.draw(textField, _matrix);
			_matrix.a = ma; _matrix.b = mb; _matrix.c = mc;
			_matrix.d = md; _matrix.tx = mtx; _matrix.ty = mty;
		}
		else
		{
	...
	{
		regenGraphic();
		// Meteoric：位图按 dpi 超采样生成（2 倍密度），绘制期按 1/dpi 临时缩回逻辑尺寸；
		// 这样 FlxText.scale 对引擎/模组的语义与原来完全一致（数值不变、显示不变）
		if (_textDpi > 1)
		{
			var scaleX:Float = scale.x;
			var scaleY:Float = scale.y;
			scale.set(scaleX / _textDpi, scaleY / _textDpi);
			super.draw();
