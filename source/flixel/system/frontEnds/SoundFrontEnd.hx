...
class SoundFrontEnd
{
	...
	// 平滑音量渐变状态（按 +/- 键时从当前值渐变到目标值）
	var _volumeFrom:Float = 0;
	var _volumeTarget:Float = 0;
	var _volumeLerp:Float = 0;
	var _volumeTweening:Bool = false;
	...
	function update(elapsed:Float):Void
	{
		...
		// 推进平滑音量渐变（约 0.15 秒，先快后慢）
		if (_volumeTweening)
		{
			_volumeLerp += elapsed / 0.15;
			if (_volumeLerp >= 1)
			{
				_volumeLerp = 1;
				_volumeTweening = false;
			}
			volume = _volumeFrom + (_volumeTarget - _volumeFrom) * easeOutCubic(_volumeLerp);
		}
