...
class LoadingState extends MusicBeatState
{
	...
	function startLoading()
	{
		...
		if (pendingChartJson != null && (PlayState.SONG == null || pendingSongName != PlayState.SONG.song))
		{
			...
			else
			{
				try
				{
					CrashHandler.mark('LoadingState:chart-parse');
					chartMessage = Song.loadFromFile(_syncChartPath, pendingChartJson == 'events');
					CrashHandler.mark('LoadingState:chart-loaded');
					onChartLoaded();
				}
				catch(e:Dynamic)
				{
	...
	function onChartLoaded()
	{
		...
		// 提前生成（先）：构建整张谱面的 CastNote + noteTypes + botplay 计划（一次遍历完成）——
		// 大谱面省去进入后 500ms+，且预渲染直接消费其 noteTypes，避免重扫整谱
		whenSharedReady(function() { startPreGen(); });

		// 提前渲染（后）：用预生成收集好的 noteTypes 烘焙音符贴图（不重扫谱面，不重复预演）；
		// 预生成关闭/失败（preGenNoteTypes 为 null）时由 Note 内部回退旧全谱扫描路径
		if (ClientPrefs.data.preRenderNotes)
		{
			var cb:Void->Void = callbacks != null ? callbacks.add('prerender') : null;
			whenSharedReady(function() {
				Note.preRenderChartNotes(PlayState.getPreGenNoteTypes());
				if (cb != null) cb();
			});
		}
		else
