...
class PlayState extends MusicBeatState
{
	...
	function startSong():Void
	{
		...
		if (chartEndTimeMs <= 0 && unspawnNotes != null && unspawnNotes.length > 0)
		{
			// 兜底再算一次（覆盖非 generateChartNotes 路径/异常早退）
			var _lastCast0:CastNote = unspawnNotes[unspawnNotes.length - 1];
			chartEndTimeMs = _lastCast0.strumTime + _lastCast0.holdLength + Conductor.safeZoneOffset + 1000;
		}
		if (chartEndTimeMs > songLength)
		{
			songLength = chartEndTimeMs;
			trace('[Audio] songLength 按谱面末尾兜底：' + Std.int(songLength) + 'ms（music.length=' + Std.int(FlxG.sound.music.length) + 'ms）');
		}
	...
	private function generateChartNotes(loadPhase:Bool):Void
	{
		...
		perfLimitNotes = ClientPrefs.data.limitNotes;
		perfHideOverlapped = ClientPrefs.data.hideOverlapped;
		var songData = SONG;
		noteTypes = [];
		noteTypeLuaPaths = [];
		noteTypeHxPaths = [];
		...
		else
		{
			...
			var res:PreGenResult = buildChartNotes(SONG, botplayPlan, isPhigrosStyle,
				{ songSpeed: songSpeed }, playbackRate, false);
			if (res.botplayLanes != null)
				Note.storeBotplayPlan(res.botplayLanes);
		...
		perfObjCap = densePerfMode ? DENSE_OBJ_CAP : 0;
		// 【Turbo】dense 档启用 notePool 复用（非密集谱恒关闭，行为与旧版一致）
		NoteGroup.poolEnabled = densePerfMode;
		...
		{
			var _lastCast:CastNote = unspawnNotes[unspawnNotes.length - 1];
			chartEndTimeMs = _lastCast.strumTime + _lastCast.holdLength + Conductor.safeZoneOffset + 1000;
		}
	...
	function noteSpawn():Void
	{
		...
		if (densePerfMode && cpuControlled && !replayMode && _bulkDrainedLast >= 128)
			_overloadFrames = 30;
		else if (_overloadFrames > 0)
			_overloadFrames--;
		_bulkDrainedLast = 0;
		// 结算 strum 高亮每帧每轨一次
		_settleAnimDone = [false, false, false, false];
		...
		var limitNotes:Int = perfLimitNotes;
		...
		if (Conductor.songPosition - lastVisBudgetCalc > 500)
		{
			...
			var probeEnd:Float = fixedPosition + (densePerfMode ? 1100 : spawnTime) * 2;
			...
			if (castsInWin > 0)
			{
				...
				var visBudget:Int = densePerfMode ? 700 : VISUAL_BUDGET;
				var visMin:Int = densePerfMode ? 1 : 3;
				var visMax:Int = densePerfMode ? 8 : 12;
				clusterVisCap = Std.int(visBudget / castsInWin);
				if (clusterVisCap > visMax) clusterVisCap = visMax;
				if (clusterVisCap < visMin) clusterVisCap = visMin;
			}
			else clusterVisCap = 12;
		}

		if (currentSpawnId == 0 && !_stageMarkSpawn)
		{
			_stageMarkSpawn = true;
		...
		var canDataSettle:Bool = densePerfMode && cpuControlled && !replayMode && _overloadFrames > 0;
		if (canDataSettle && !isTurboAggregateIndex(currentSpawnId))
		{
			var settleCut:Float = fixedPosition + spawnWindowFor(unspawnNotes[currentSpawnId]) * 0.5;
			var settleBudget:Int = Std.int(Math.max(4096, Math.min(65536, FlxG.elapsed * 1000 * 100)));
			var settled:Int = 0;
			while (currentSpawnId < unspawnNotes.length && settled < settleBudget)
			{
				var sTarget:CastNote = unspawnNotes[currentSpawnId];
		...
		while (currentSpawnId < unspawnNotes.length && processed < spawnBudget)
		{
			...
			if (target.wasHit) { currentSpawnId++; continue; } // 【Turbo】已数据层结算：跳过不物化

			// 【密集对象硬上限】场上精灵对象数达到上限时，距判定线仍 > DENSE_OBJ_LATE_MS
			// 的箭头延迟进场（光标不退、时间推进后自然放行；到窗口内即使超限也生成，
			// 判定/计分/排期命中零影响）。仅视觉上"墙更贴线"，与用户拍板的预期一致。
			if (perfObjCap > 0 && notes.aliveObjectCount >= perfObjCap)
			{
				var lateWin:Float = DENSE_OBJ_LATE_MS * playbackRate;
				if (songSpeed < 1) lateWin /= songSpeed;
		...
		releaseConsumedNotes(); // 【性能】清扫已消费 CastNote（游标左侧），释放 GC 引用
	...
	function spamSpawn():Void
	{
		...
		for (spam in spamNotes)
		{
			...
			while (isDisplay && spam.remaining > 0 && guard < 8192)
			{
				...
				if (perfLimitNotes > 0 && notes.countLiving() >= perfLimitNotes) break;
				spam.remaining--;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && !ClientPrefs.data.hudOnly && dad.animOffsets.exists('hey')) {
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!(PlayState.densePerfMode && (cpuControlled || replayMode)))
		...
		if (!note.wasGoodHit)
		{
			...
			{
				var stackGainPct:Float = (note.hitHealth * healthGain * hitMult) / 2 * 100;
				pendingFlySpikePct = Math.max(pendingFlySpikePct, Math.min(100 + stackGainPct, 1000));
			}
			// Bad 及以下评分扣一点点血
			...
			if(!ClientPrefs.data.hudOnly && !note.noAnimation && !(botHitBatch && botBatchAnimDone[note.noteData])) {
			...
			else if (!(botHitBatch && botBatchAnimDone[note.noteData]) && ClientPrefs.data.botLightStrum)
