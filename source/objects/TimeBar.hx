...
/**
 * 独立的时间进度条组件。
 * 底框（timeBar.png）与双色填充由本对象一次性绘制，
 * 不再需要额外的 timeBarBG 精灵，从根本上避免出现两条进度条。
 */
class TimeBar extends FlxSpriteGroup
{
	...
	// 与 HealthBar 保持一致的公开属性，供旧模组脚本读取/设置
	public var barWidth(default, set):Int = 1;
	public var barHeight(default, set):Int = 1;
	public var barOffset:FlxPoint = new FlxPoint(3, 3);
	public var borderSize(default, set):Int = 0;
	public var hollowShape:Bool = false;
	private var modernMask:BitmapData = null;

	public function new(x:Float, y:Float, ?valueFunction:Void->Float, boundX:Float = 0, boundY:Float = 1, ?modern:Bool = false)
	{
		super(x, y);
		...
		if (modern)
		{
			bg = generateModernBackground();
		}
		else
		{
			bg = new FlxSprite().loadGraphic(Paths.image('timeBar'));
		...
		if (modernMask != null)
		{
			// 新样式：黑色圆角底 + 内缩 2px 的圆角填充（四周留出小黑边）
			leftBar.pixels = modernMask.clone();
			rightBar.pixels = modernMask.clone();
			rightBar.color = FlxColor.BLACK;
			barWidth = Std.int(bg.width) - 4;
			barHeight = Std.int(bg.height) - 4;
			barOffset.set(2, 2);
	...
	// 新样式：纯代码绘制的实心黑色圆角背景，另生成内缩 2px 的填充遮罩（四周小黑边）
	private function generateModernBackground():FlxSprite
	{
		var w:Int = 600;
		var h:Int = 19;
		var border:Int = 2;

		var maskBitmap:BitmapData = new BitmapData(w, h, true, 0x00000000);
		var maskShape:Shape = new Shape();
		maskShape.graphics.beginFill(0xFFFFFFFF, 1);
		maskShape.graphics.drawRoundRect(border, border, w - border * 2, h - border * 2, 6, 6);
		maskShape.graphics.endFill();
		maskBitmap.draw(maskShape);
		modernMask = maskBitmap;

		var bitmap:BitmapData = new BitmapData(w, h, true, 0x00000000);
		var shape:Shape = new Shape();
		shape.graphics.beginFill(0xFF000000, 1);
		shape.graphics.drawRoundRect(0, 0, w, h, 8, 8);
		shape.graphics.endFill();
		bitmap.draw(shape);

		var spr:FlxSprite = new FlxSprite().makeGraphic(w, h, FlxColor.TRANSPARENT);
		spr.pixels = bitmap;
		return spr;
	}
