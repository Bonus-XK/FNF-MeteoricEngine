...
class CastNote
{
	...
	static var __wasHit:Array<Bool> = [];
	...
	public static function resetPacked():Void
	{
		...
		__angle = []; __offsetAngle = []; __wasHit = [];
	}
	...
	// 【Turbo 数据级结算】已由 fastSkip 数据层结算（计分/命中/血量已入账，不再物化 Sprite）
	public var wasHit(get, set):Bool;
	inline function get_wasHit():Bool return __wasHit[idx];
	inline function set_wasHit(v:Bool):Bool return __wasHit[idx] = v;
}
...
class Note extends FlxSprite
{
	...
	// 【密集批·延迟移除】processBotHits 批量击杀期间登记（true=本帧帧末由 NoteGroup.endBatchKill 统一清扫）
	public var batchKillPending:Bool = false;
	...
	private function set_noteType(value:String):String {
		...
		if(noteData > -1 && noteType != value) {
			// 仅类型真正变化时重刷轨道色：null→null（池化复位）不再触发 defaultRGB()，
			// 避免它在 rgbShader 禁用且共享状态下把颜色写进 Note.globalRgbShaders[0]
			// （左箭头全局调色板），造成"左箭头按下有时变绿/蓝/红"。
			// 池化复生的轨道色由 recycleNote 在 RGB 启用状态重建后显式 defaultRGB() 负责（已克隆，安全）。
			defaultRGB();
			switch(value) {
				case 'Hurt Note':
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false, ?createdFrom:Dynamic = null)
	{
		...
		if(noteData > -1) {
			...
			else if(ClientPrefs.data.psych063Mode || !ClientPrefs.data.shaders || PlayState.densePerfMode) rgbShader.enabled = false; // 0.6.3 兼容/关着色器/密集谱自动档：避免 RGB 渲染成黑或叠色
			#if mobile
			...
			// 【视觉修正】仅用户手动开启 perfMode 才平涂；密集谱自动档（densePerfMode）改用 063 原生
			// 贴图 + 关 RGB，绝不再叠平涂色（否则烘焙色×平涂色双重着色 → "阴间"配色）。
	...
	public function reloadNote(texture:String = '', postfix:String = '') {
		...
		{
			var useRGB:Bool = !PlayState.densePerfMode && !(PlayState.SONG != null && PlayState.SONG.disableNoteRGB == true);
	...
	static function getNoteSkinLoadPath(texture:String, postfix:String = '', pixelStage:Bool = false, isSustain:Bool = false):String
	{
		...
		// 【视觉修正】密集谱自动档：默认皮肤（内置 NOTE_assets）强制换 PE 0.6.3 原生贴图
		// （noteSkins/NOTE_assets-063，引擎自带 PNG+XML）：贴图自带四色，配"关 RGB + 不叠平涂色"
		// 得到干净的原版配色，同时省去烘焙着色/着色器开销。模组显式 arrowSkin / 用户自定义
		// noteSkin 时保持不动（尊重模组皮肤）。
		if (PlayState.densePerfMode && !pixelStage && skin == 'noteSkins/NOTE_assets')
			skin = 'noteSkins/NOTE_assets-063';
	...
	// 加载曲目时调用：烘焙所有可能出现的音符贴图。
	// providedTypes != null：由预生成（buildChartNotes）一次遍历收集好的 noteType 列表，
	// 直接消费 → 不再二次扫描整张谱面，也不再跑 preRenderBotplay（计划已随预生成存入
	// Note 静态槽）；预生成关闭/失败（null）时回退旧全谱扫描 + preRenderBotplay 路径。
	public static function preRenderChartNotes(?providedTypes:Array<String>):Void
	{
		...
		var types:Array<String> = providedTypes;
		if (types != null)
		{
		...
		try
		{
			for (type in types)
			{
				for (col in 0...4)
				{
					...
					var useRGB:Bool = !PlayState.densePerfMode && !(PlayState.SONG.disableNoteRGB == true);
		...
		// 计划已由预生成（buildChartNotes）存入时不再重复扫描；仅旧路径（无预生成）兜底
		...
		if (PlayState.SONG != null && PlayState.SONG.notes != null)
		{
			...
			for (section in PlayState.SONG.notes)
			{
				...
				for (songNotes in section.sectionNotes)
				{
					...
					// Psych 1.0.4 / convertToPsychV1 / fastParseSong 后列号为绝对方向（<4 玩家，>=4 对手），
					// 与 PlayState.buildChartNotes 的判定完全一致；旧的 mustHitSection 翻转规则已作废
					// （否则 mustHit=false 段的玩家音符被漏计 → botplayPlan 校验失败 → 退回实时判定，
					//   高密度谱自动命中路径失效）。Obsolescence-spam 类全绝对列号谱面必须走本规则。
	...
	// 由预生成（PlayState.buildChartNotes）存入自动游玩计划：绝对列号规则 + 与
	// botLaneCounts 同口径（含长条段与密度倍数），取代 preRenderBotplay 的二次全谱扫描
	public static function storeBotplayPlan(plan:Array<Array<Float>>):Void
	{
		botplayLanes = plan;
		botplaySignature = currentChartSignature();
	}
	...
	override function draw():Void
	{
			...
			&& !flipX && !flipY && blend == null && shader == null)
		{
			@:privateAccess
			{
				...
				{
					// 【性能榨干】tile 渲染下 calcFrame(false) 是纯 no-op
					// （FlxSprite.calcFrame 在 FlxG.renderTile && !force 时直接 return），
					// 逐音符每帧白调一次方法+一次内部 checkEmptyFrame；仅 useFramePixels
					// 需要重绘帧像素时才强制 calcFrame(true)（语义与原调用完全等价）。
					if (useFramePixels && dirty)
						calcFrame(true);
	...
	public function recycleNote(target:CastNote):Note
	{
		...
		// ★必须放在上面 RGB 启用状态重建之后：提前渲染路径的模板 rgbShader 为【禁用且共享】
		// （enabled=false、parent=Note.globalRgbShaders[0]）。若在此前置空，
		// set_noteType(null) 顶部的 defaultRGB() 会因禁用态不克隆而把当前复生轨道的颜色
		// 直接写进左箭头的共享调色板 → 左键按下“有时变色”（绿/蓝/红/紫轮流）。
		// 重建后置空：enabled=true → 写色前先克隆私有调色板，共享调色板永不被污染。
		noteType = null;
		...
		// 【视觉修正】仅 perfMode 平涂；密集谱自动档（densePerfMode）不叠色（见构造函数注释）
		if (ClientPrefs.data.perfMode)
		{
			rgbShader.enabled = false;
			var laneColors:Array<FlxColor> = PlayState.isPixelStage ? ClientPrefs.data.arrowRGBPixel[noteData] : ClientPrefs.data.arrowRGB[noteData];
	...
	public function followStrumNote(myStrum:StrumNote, fakeCrochet:Float, songSpeed:Float = 1)
	{
		...
		if (copyAngle)
		{
			// 【性能榨干】密集谱绝大多数音符方向恒为 0（direction=90、无旋转/无 offsetAngle）：
			// 数值不变时跳过 setter 写入（set_angle 每次都会重算弧度/标记 dirty），
			// 每音符每帧省一次属性写；语义/引擎每帧公式标记逻辑不变。
			var newAngle:Float = strumDirection - 90 + strumAngle + offsetAngle;
			if (angle != newAngle)
			{
		...
		if(copyAlpha)
		{
			var newAlpha:Float = strumAlpha * multAlpha;
			if (alpha != newAlpha) alpha = newAlpha;
		}
