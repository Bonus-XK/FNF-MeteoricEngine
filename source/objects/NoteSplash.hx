...
class NoteSplash extends FlxSprite
{
	...
	public static var defaultNoteSplash(default, never):String = 'noteSplashes/noteSplashes-063';
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null) {
		...
		// PE 0.6.3 经典预染色皮肤（noteSplashes-063）：贴图自带四色（紫/蓝/绿/红），
		// 直接 raw 显示即原版观感——不再走 PixelSplashShader（RGB 通道加权贴图是
		// "五颜六色"乱象的来源），同时省掉每颗溅射的 shader 绑定/uniform 开销。
		// 自定义 splashSkin（模组提供、且能成功加载）仍走原 RGB 着色路径，行为不变。
		var use063Raw:Bool = (texture != null && texture.indexOf('noteSplashes-063') >= 0)
			|| (_configLoaded != null && _configLoaded.indexOf('noteSplashes-063') >= 0);

		var tempShader:RGBPalette = null;
		if(!use063Raw && (note == null || note.noteSplashData.useRGBShader) && (PlayState.SONG == null || !PlayState.SONG.disableNoteRGB))
		{
		...
		// 063 raw：解绑着色器直接显示预染色贴图（copyValues(null) 只是把 mult 置 0，
		// shader 仍挂载；这里彻底移除保证零着色开销与像素块映射不介入）
		shader = use063Raw ? null : rgbShader.shader;
