...
class NoteOffsetState extends MusicBeatState
{
	...
	function updateNoteDelay()
	{
		...
		timeTxt.text = '切换延迟：' + Math.floor(barPercent) + ' ms';
	}
	...
	function updateMode()
	{
		...
		if(onComboMenu)
			changeModeText.text = '< Combo偏移（按空格切换） >';
		else
			changeModeText.text = '< 音符/节拍延迟（按下空格进行切换） >';
