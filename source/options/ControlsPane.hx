...
class ControlsPane extends SettingsCanvas
{
	var wheelScroll:WheelScroll = new WheelScroll(); // 滚轮限速（Freeplay 同款）
	// ===== 布局（**实例字段**：本画布既服务暂停内嵌整屏路径，也服务单界面设置的 956×570 内容区）=====
	// 两条路径共用同一份代码与内部逻辑，只是几何基准不同：
	//   hosted=false（rect=null，默认）：沿用改造前的整屏常量（120,70,1040,580 + 背景 + 底部条 + 返回键），像素级不变；
	//   hosted=true：面板 = 宿主内容区 rect，清单左缩进、键位列右贴（换算见 new()），
	//                不再重复画背景/底部条/返回键/标题（那些由单界面设置宿主提供）。
	...
	var PANEL_X:Float = 120;
	var PANEL_Y:Float = 70;
	var PANEL_W:Float = 1040;
	var PANEL_H:Float = 580;

	var LIST_X:Float = 160;     // 名称文字 X
	var LIST_Y:Float = 170;     // 第一行 Y
	var ROW_GAP:Float = 52;     // 行距
	var ROWS_VISIBLE:Int = 9;   // 可见行数

	var KEY_X:Float = 700;      // 主键位框 X
	var KEY_GAP:Float = 250;    // 两个键位框间距
	var KEY_W:Float = 210;
	var KEY_H:Float = 42;
	 ...
	 *                 传入矩形时本画布不再自绘背景/底部条/返回键/标题 —— 那些由单界面设置宿主提供。
	 */
	public function new(?hostRect:FlxRect = null)
	{
		...
		if (hostRect != null)
		{
			...
			PANEL_X = hostRect.x;
			PANEL_Y = hostRect.y;
			PANEL_W = hostRect.width;
			PANEL_H = hostRect.height;
			LIST_X = PANEL_X + 40;                          // 324（整屏时为 160 = 120+40，同一缩进规则）
			LIST_Y = PANEL_Y + 100;                         // 170（与整屏版同一行首）
			// 键位列右贴：两框总宽 = KEY_GAP + KEY_W = 460，右侧留 40 → KEY_X = 内容区右缘 − 500
			KEY_X = PANEL_X + PANEL_W - 40 - (KEY_GAP + KEY_W);
		}
		else
		{
		...
		// ---- 模式切换标签（键盘/手柄）：贴面板右缘（整屏 = 820/1030，与原实现一致；内容区按同一右侧偏移换算）----
		var linksRight:Float = PANEL_X + PANEL_W;
		addModeLink(linksRight - 340, 106, '> 键盘模式');
		addModeLink(linksRight - 130, 106, '> 手柄模式');
		refreshModeLabels();
	...
	{
		// 单界面宿主：焦点不在内容区（用户在分区栏上浏览）时画布只绘制、不吃输入，
		// 否则 ↑↓/Esc 会被"画布内部列表"与"分区栏"同时消费，出现两处一起动的错乱感
		if (!inputEnabled)
		{
		...
		// 返回键：整屏画布自绘；内容区画布的返回键由单界面设置宿主提供（backBtn 为 null，跳过即可）
		if (backBtn != null)
		{
	...
	function startBinding()
	{
		...
		var pw:Float = 640;
		var ph:Float = 280;
		// 整屏：屏幕居中；单界面宿主：内容区居中（弹层必须落在自己那块画布里）
		var px:Float = hosted ? (PANEL_X + (PANEL_W - pw) / 2) : (FlxG.width - pw) / 2;
		var py:Float = hosted ? (PANEL_Y + (PANEL_H - ph) / 2) : (FlxG.height - ph) / 2;
	...
	function exitState()
	{
		...
		FlxG.sound.play(Paths.sound('cancelMenu'));
		// 旧路径（暂停内嵌/整屏）= 关闭子状态；单界面宿主 = 焦点回到分区栏（由宿主注入 onExit）
