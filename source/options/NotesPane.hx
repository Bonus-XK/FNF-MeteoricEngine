...
class NotesPane extends SettingsCanvas
{
	var wheelScroll:WheelScroll = new WheelScroll(); // 滚轮限速（Freeplay 同款）
	// ===== 布局（**实例字段**：既服务暂停内嵌的整屏画布，也服务单界面设置的 956×570 内容区）=====
	//   hosted=false（rect=null，默认）：沿用改造前的整屏常量，像素级不变；
	//   hosted=true：左右面板按内容区矩形重排（左 560 宽 / 右 382 宽 / 中间 14 间距），
	//                模式列与箭头列在左面板内**居中**；色轮缩到 250 并把亮度渐变条移到色轮**右侧**
	//                （382 宽装不下原始横向排布"渐变条 46 + 16 间距 + 色轮 300"）。
	var hosted:Bool = false;
	// FlxGroup 没有 controls 取值器（MusicBeatState/MusicBeatSubstate 才自带），按同一模式接上
	var controls(get, never):Controls;
	inline function get_controls():Controls
		return Controls.instance;

	var PANEL_L_X:Float = 40;
	var PANEL_L_Y:Float = 70;
	var PANEL_L_W:Float = 680;
	var PANEL_L_H:Float = 570;

	var PANEL_R_X:Float = 740;
	var PANEL_R_Y:Float = 70;
	var PANEL_R_W:Float = 460;
	var PANEL_R_H:Float = 570;

	var MODE_X:Float = 227;
	var MODE_Y:Float = 165;
	var MODE_GAP:Float = 110;
	var MODE_SIZE:Float = 85;

	var NOTE_X:Float = 177;
	var NOTE_Y:Float = 305;
	var NOTE_GAP:Float = 135;
	var NOTE_SIZE:Float = 102;

	var WHEEL_X:Float = 820;
	var WHEEL_Y:Float = 170;
	var WHEEL_SIZE:Float = 300;

	var GRADIENT_X:Float = 758;
	var GRADIENT_Y:Float = 170;
	var GRADIENT_W:Float = 46;
	var GRADIENT_H:Float = 300;

	var PALETTE_X:Float = 776;
	var PALETTE_Y:Float = 568;
	var PALETTE_SCALE:Float = 18;
	...
	/**
	 * @param hostRect 单界面设置的内容区矩形（null = 暂停内嵌的整屏画布，几何与改造前逐像素一致）。
	 *                 传入矩形时不画背景/面板/标题/底部条/返回键 —— 那些由单界面设置宿主提供。
	 */
	public function new(?hostRect:FlxRect = null)
	{
		super();
		...
		if (hostRect != null)
		{
			// ---- 单界面设置（内容区）：按内容区矩形重排 ----
			hosted = true;
			PANEL_L_X = hostRect.x;
			PANEL_L_Y = hostRect.y;
			PANEL_L_W = 560;
			PANEL_L_H = hostRect.height;
			PANEL_R_X = hostRect.x + 574;              // 左面板右缘(844) + 14 间距
			PANEL_R_Y = hostRect.y;
			PANEL_R_W = hostRect.width - 574;          // 382
			PANEL_R_H = hostRect.height;

			// 模式列 / 箭头列在左面板内**居中**（跨度 = (n−1)×GAP + SIZE；原整屏版同样是居中放置）
			MODE_X = PANEL_L_X + (PANEL_L_W - (MODE_GAP * 2 + MODE_SIZE)) / 2;
			NOTE_X = PANEL_L_X + (PANEL_L_W - (NOTE_GAP * 3 + NOTE_SIZE)) / 2;

			// 右面板只有 382 宽：色轮 300 → 250，亮度渐变条（46 宽）改放色轮**右侧**
			// （原始的"渐变条在色轮左侧"需要 62px 边距，窄面板放不下）
			WHEEL_SIZE = 250;
			WHEEL_X = PANEL_R_X + 66;
			GRADIENT_X = PANEL_R_X + PANEL_R_W - GRADIENT_W - 18;
			GRADIENT_H = WHEEL_SIZE;
			PALETTE_X = PANEL_R_X + 36;
		}
		else
		{
		...
		// ---- 右侧：复制 / 粘贴（贴右面板右缘；整屏 = 1004/1080，与原实现一致）----
		copyButton = new FlxSprite(PANEL_R_X + PANEL_R_W - 196, 92).loadGraphic(Paths.image('noteColorMenu/copy'));
		copyButton.alpha = 0.6;
		add(copyButton);

		pasteButton = new FlxSprite(PANEL_R_X + PANEL_R_W - 120, 92).loadGraphic(Paths.image('noteColorMenu/paste'));
		...
		// ---- RGB 数值 / HEX 输入：整屏按原坐标；内容区右面板窄 78px → 横向按面板宽度等比压缩（同一 3 列结构）----
		var rx:Float = PANEL_R_W / 460; // 横向压缩比（整屏 = 1.0，内容区 ≈ 0.83）
		alphabetR = makeColorAlphabet(PANEL_R_X + 52 * rx, 486);
		alphabetG = makeColorAlphabet(PANEL_R_X + 197 * rx, 486);
		alphabetB = makeColorAlphabet(PANEL_R_X + 342 * rx, 486);

		var labR:MenuText = makeColorAlphabet(PANEL_R_X + 22 * rx, 487, 0.35);
		labR.text = 'R';
		labR.color = 0xFFFF6B6B;
		labR.updateHitbox();
		var labG:MenuText = makeColorAlphabet(PANEL_R_X + 167 * rx, 487, 0.35);
		labG.text = 'G';
		labG.color = 0xFF6BFF8F;
		labG.updateHitbox();
		var labB:MenuText = makeColorAlphabet(PANEL_R_X + 312 * rx, 487, 0.35);
		labB.text = 'B';
		labB.color = 0xFF6BB5FF;
		labB.updateHitbox();

		// ---- HEX 输入 ----
		alphabetHex = makeColorAlphabet(PANEL_R_X + 72, 534, 0.5);
		var labHex:MenuText = makeColorAlphabet(PANEL_R_X + 22, 536, 0.35);
		labHex.text = 'HEX';
		...
		if (hosted) // 内容区：光标初始落在内容面板中心（不再用整屏 screenCenter）
		{
			controllerPointer.x = PANEL_L_X + (PANEL_R_X + PANEL_R_W - PANEL_L_X) / 2;
			controllerPointer.y = PANEL_L_Y + PANEL_L_H / 2;
		}
		else controllerPointer.screenCenter();
	...
	{
		// 单界面宿主：焦点不在内容区（用户在分区栏上浏览）时画布只绘制、不吃输入，见 ControlsPane 同款说明
		if (!inputEnabled)
		{
		...
		if (controls.BACK)
		{
			...
			if (onExit != null) onExit(); // 整屏路径 = close()；单界面宿主 = 焦点回分区栏
			return;
		}
		...
		// 返回按钮（整屏画布自绘；内容区画布的返回键由单界面设置宿主提供：backBtn 为 null）
		if (backBtn != null)
		{
